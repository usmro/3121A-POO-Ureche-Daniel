...
public:
  Vehicul(const std::string &id, double vitezaMaxima)
      : id(id), vitezaMaxima(vitezaMaxima), indexRuta(0),
        progresPeStradaCurenta(0.0) {}
  virtual ~Vehicul() = default;

  virtual double calculeazaTimpParcurgere(double lungimeStrada,
                                          double limitaVitezaStrada) const = 0;
  ...
  Strada *getStradaCurenta() const {
    if (indexRuta < rutaCurenta.size())
      return rutaCurenta[indexRuta];
    return nullptr;
  }

  double getProgresPeStradaCurenta() const { return progresPeStradaCurenta; }
  ...
  bool avanseaza(double dt, double limitViteza) {
    ...
    double vitezaReala =
        (lungimeStrada_helper(s) /
         calculeazaTimpParcurgere(s->getLungime(), limitViteza));
    // Viteza reala [m/s]. In loc de acest calcul, luam direct o viteza bazata
    // pe polimorfism.
...
private:
  double lungimeStrada_helper(Strada *s) const {
    return s ? s->getLungime() : 1.0;
  }
};
