...
SimulareEngine::SimulareEngine() : isRunning(false), isPaused(false), speedMultiplier(1) {
...
void SimulareEngine::adaugaVehicul(std::unique_ptr<Vehicul> v) {
    if (v) vehiculeInAsteptare.push_back(std::move(v));
}
...
void SimulareEngine::setViteza(int m) { if (m > 0) speedMultiplier = m; }
    ...
    while (isRunning) {
        if (!isPaused) {
            ...
            if (vehiculeActive.empty() && !vehiculeInAsteptare.empty()) {
                ...
                std::cout << "[Batching] Introducem " << masiniDeAdaugat << " vehicule noi pe harta.\n";
                ...
                for (int i = 0; i < masiniDeAdaugat; ++i) {
                    ...
                    std::cout << "[Bariere] Deschidere " << start << " pt intrare vehicul " << v->getId() << "\n";
            ...
            for (auto it = vehiculeActive.begin(); it != vehiculeActive.end(); ) {
                ...
                if (!stradaCurenta) {
                    ...
                    // Trebuie sa verificam care a fost ultima strada pentru a sti ce bariera sa deschidem
                    // Alternativ, ne putem uita la destinatia initiala a vehiculului (cum a fost calculata ruta).
                    // Momentan simulam deschiderea barierei corespunzatoare 
                    // Daca s-a terminat pe S1 sau S1_inv, e B1. Daca e S6, e B2. (Sau pur si simplu le deschidem pe ambele scurt pentru test)
                ...
                // Calcul viteza
                double timpMax = v->calculeazaTimpParcurgere(stradaCurenta->getLungime(), stradaCurenta->getLimitaViteza());
                double vitezaMps = stradaCurenta->getLungime() / timpMax;
                
                bool aTerminatStrada = v->avanseaza(dt, stradaCurenta->getLimitaViteza());
                ...
                if(nume.find("S1") != std::string::npos) idHardware = 0;
                else if(nume.find("S2") != std::string::npos) idHardware = 1;
                else if(nume.find("S3") != std::string::npos) idHardware = 2;
                else if(nume.find("S4") != std::string::npos) idHardware = 3;
                else if(nume.find("S5") != std::string::npos) idHardware = 4;
                else if(nume.find("S6") != std::string::npos) idHardware = 5;
                
                if(nume.find("_inv") != std::string::npos) invers = true;
                
                if (idHardware != -1) {
                    double progres = v->getProgresPeStradaCurenta() / stradaCurenta->getLungime();
                    int indexLed = (int)(progres * 8.0);
                    if (indexLed >= 8) indexLed = 7;
                    if (invers) indexLed = 7 - indexLed; // Merge invers fizic pe LED-uri
                    
                    hwBridge->setLedStatus(idHardware, indexLed, true);
                }

                if (aTerminatStrada) {
                    std::cout << "> [" << v->getId() << "] a terminat " << stradaCurenta->getNume() << "\n";
                }
                ++it;
            }
        } else {
        ...
        std::this_thread::sleep_for(std::chrono::milliseconds(100)); // Tick la 100ms
    }
}
